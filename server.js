...
const Permission = (req, res, next) => {
  ...
  try {
    ...
    res
      .status(401)
      .json({ error: "Token không đúng, vui lòng đăng nhập" + error });
  }
};
// Router
server.post("/create-collection", (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to update db.json", details: error.message });
  }
});
...
server.get("/products", (req, res) => {
  ...
  const result = simpleproduct.map((item) => {
    const children = products.filter(
      (child) => child.parent == item.id && child.type == "product_variation"
    );
...
server.get("/products/:id", (req, res) => {
  ...
  const children = products.filter(
    (child) => child.parent == id && child.type == "product_variation"
  );
  if (children.length > 0) {
    const pricearr = children.map((child) => child.price);
    product.price =
...
server.post("/carts", Permission, (req, res) => {
  ...
  if (index == -1) {
    ...
    res
      .status(200)
      .json({ message: "Thêm giỏ hàng thành công!", data: newcart });
  } else {
    const item = db.carts[index].Items.filter(
...
server.put("/carts", Permission, (req, res) => {
  ...
  res
    .status(200)
    .json({ data: db.carts[index], message: "Cập nhật thành công" });
});
...
server.listen(port, () => {
  ...
  console.log(
    `Tạo mới collection: http://localhost:${port}/create-collection =>Method: POST`
  );
  GetEndpoint();
});
